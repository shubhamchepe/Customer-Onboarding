...
using NICModel = PMEGPCUSTOMERBank.Modal.NICModel;

namespace PMEGPCUSTOMERBank;

public partial class RegisteredApplicantsDashboardView : ContentView
{
    ...
    public List<Modal.NICModel> PagedList { get; set; } = new List<Modal.NICModel>();
    ...
    public RegisteredApplicantsDashboardView()
	{
		InitializeComponent();
    ...
    private async void LoadIndustries()
    {
        try
        {
            ...
            _allIndustries = System.Text.Json.JsonSerializer.Deserialize<List<NICModel>>(response) ?? new List<NICModel>();
            int sr = 1;
            foreach (var item in _allIndustries)
                item.SrNo = sr++;

            // 👇 Bind directly
            // industryCollection.ItemsSource = _allIndustries;
    ...
    private async void ClassPicker_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (classPicker.SelectedItem is ClassModel selectedClass && !string.IsNullOrEmpty(selectedClass.NicCode))
        {
            ...
            var industries = JsonConvert.DeserializeObject<List<NICModel>>(response);
    ...
    private void OnSelectClicked(object sender, EventArgs e)
    {
        ...
        int srNo = SelectedIndustries.Count + 1;
        foreach (var item in selected)
        {
            if (!SelectedIndustries.Any(x => x.NicCode == item.NicCode))
            {
                item.SrNo = srNo++;
        ...
        // ✅ Close popup (Grid invisible)
        industryPopupContainer.IsVisible = false;
    }
    ...
    {
        var source = _isFilterApplied ? _currentSource : _allIndustries;
        ...
        industryCollection.ItemsSource = pagedData;

        int totalPages = (int)Math.Ceiling((double)source.Count / _pageSize);
    ...
    private void OnClearClicked(object sender, EventArgs e)
    {
        ...
        _isFilterApplied = false;   // reset to initial data
        _currentPage = 1;
        LoadPage();
    }
    private void OnNextClicked(object sender, EventArgs e)
    {
        if (_currentPage * _pageSize < _allIndustries.Count)
        {
            _currentPage++;
            LoadPage();
        }
    ...
    {
        // close popup if user clicks outside frame
        industryPopupContainer.IsVisible = false;
    }

    private void OnPopupContentTapped(object sender, TappedEventArgs e)
    {
        // do nothing -> this stops event bubbling to background
    }
    ...
    private async void OnSelectIndustryClicked(object sender, EventArgs e)
    {
        ...
        // Show popup inline
        industryPopupContainer.IsVisible = true;
        if (selectedScheme == null || selectedScheme.ActivityTyp == 0)
        {
            await Application.Current.MainPage.DisplayAlert("Error", "Please select an Activity Type first.", "OK");
            return;
        }
    ...
    private async void OnSelectEdpClicked(object sender, EventArgs e)
    {
        ...
        var selectedOffice = await Application.Current.MainPage.ShowPopupAsync(popup) as EdpOfficeModel;
